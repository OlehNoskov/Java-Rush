...
public class Server {
    ...
    private static class Handler extends Thread {
        ...
        public static void sendBroadcastMessage(Message message) {
            //Рассылаем сообщения по всем соединениям
            for (Connection connection : connectionMap.values()) {
                //Прохожу в Map по соединениям!
                try {
                    connection.send(message);
                } catch (IOException e) {
                    ConsoleHelper.writeMessage("Вы не смогли отправить сообщение! " + connection.getRemoteSocketAddress());
                }
            }
        }
